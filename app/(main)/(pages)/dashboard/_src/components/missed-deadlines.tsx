    ...
    <Card className="border-border/80 hover:border-border hover:shadow-md transition-all duration-200 group w-full">
      ...
      <CardContent className="space-y-4">
        {projectsWithDaysLeft.map((project, index) => (
          ...
          >
            <div className="hidden sm:flex h-8 w-8 rounded-full flex items-center justify-center sm:mr-2 bg-gray-700 dark:bg-gray-300">
        ...
        {projectsWithDaysLeft.length === 0 && <p className="text-foreground text-center">No missed deadlines 🎉</p>}
      </CardContent>
    </Card>
  );
}
