...
export function ResponseTimeCard() {
  ...
  const formattedResponseTime = data?.avgResponseTime !== undefined ? 
    Number.isInteger(data.avgResponseTime) ? 
      `${data.avgResponseTime}` : 
      `${Number(data.avgResponseTime).toFixed(1)}` : 
    "--";
        ...
        <div className="flex items-center justify-between">
              ...
              <div className={`flex items-center text-sm ${isFasterResponse ? "text-green-500 dark:text-green-400" : "text-red-500 dark:text-red-400"}`}>
                {responseTimeChange !== 0 ? (
                  <>
                    ...
                    <span>
                      {`${Math.abs(responseTimeChange)}% ${isFasterResponse ? "improvement" : "decrease"}`}
                    </span>
                  </>
                ) : (
                  <> 
          ...
          <div className="relative h-12 w-12 flex items-center justify-center">
            <div className="relative p-2 rounded-full">
              ...
              <div 
                className={`absolute inset-0 rounded-full border-2 border-transparent ${isFasterResponse ? 'border-green-400/50' : 'border-red-400/50'}`}
              />
              <div 
