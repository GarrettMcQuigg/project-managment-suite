...
export async function CalculateAverageResponseTime(userId: string): Promise<void> {
  try {
    ...
    if (!userAnalytics?.communicationAnalytics) {
      console.log('Communication analytics not found for user:', userId);
      return;
    }
      ...
      for (const message of projectMessages) {
        ...
        if (isUserMessage) {
          if (lastClientMessage) {
            const responseTimeMinutes = Math.round((message.createdAt.getTime() - lastClientMessage.getTime()) / (1000 * 60));
    ...
    if (responseCount > 0) {
      ...
      if (userAnalytics.communicationAnalytics.avgResponseTime === 0 || userAnalytics.communicationAnalytics.avgResponseTime === null) {
        await db.communicationAnalytics.update({
          where: { id: userAnalytics.communicationAnalytics.id },
          data: {
